...
class YoloModel(Base):
    ...
    metrics = Column(JSON, nullable=True, comment='模型指标（验证集）')
    test_metrics = Column(JSON, nullable=True, comment='测试集指标')
    test_status = Column(String(20), default='untested', comment='测试集评估状态(untested/pending/running/completed/failed)')
    classes = Column(JSON, nullable=False, comment='类别列表')
    created_at = Column(DateTime, server_default=func.now(), comment='创建时间')
    is_deleted = Column(Integer, default=0, comment='是否删除(0=未删除, 1=已删除)')
