...
| **动态设备分配** | `plan_case_relation` 表支持设备为空，关联用例时可选"动态分配"，由系统自动调度空闲设备 |
| **数据库迁移** | 新增 `migration_002` 脚本，修复 `device_id` 等列为 NOT NULL 导致的动态分配报错 |

### 2026年7月 — YOLO 评估与 UI 优化

| 类别 | 更新内容 |
|------|---------|
| **YOLO 测试集评估** | 支持 YOLO 模型测试集评估功能，训练前检查验证集并提示确认，支持评估任务取消 |
| **YOLO 灵活接入** | YOLO 模型可选为空，降级为纯 OCR 识别；弹窗确认后继续关联 |
| **测试任务体验** | 测试任务列表改用弹窗展示 Job 详情；监控页增加 YOLO/OCR 模型显示 |
| **创建人修复** | 测试任务创建人从写死改为当前登录用户 |
| **取消任务** | 支持取消测试集评估任务，后台检查状态和取消信号 |
| **UI 优化** | 测试集指标卡片 SwiftUI 风格；标注提示信息可关闭；取消按钮 hover 交互优化 |
| **文档完善** | 补充系统截图、部署说明、数据库说明、API 概览、快速启动指南 |
