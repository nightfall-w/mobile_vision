...
| 类别 | 更新内容 |
|------|---------|
| **执行回放** | 支持 3X 倍速回放整个测试执行过程，根据实际截图时间差动态计算播放速度 |
| **历史截图查看** | 点击历史步骤即可查看该步骤执行时的设备截图，支持"返回实时"一键切换 |
| **主动截图关联** | 每个 step 执行完成后主动截取设备截图并关联到对应步骤，确保回放帧数完整 |
| **子任务状态同步** | 回放时子任务区域自动同步高亮（已完成收起/当前展开/自动滚动）、日志按时间轴逐步显示 |
| **截图推送提速** | 截图推送频率统一从 1.5-2s 提升为 1s，监控页实时性更佳 |
| **动态设备分配** | 关联用例时设备可选为空，系统自动调度空闲设备 |
| **数据库迁移** | 新增 `migration_002_plan_case_relation_nullable.sql`，`plan_case_relation` 表 `device_id` / `device_name` / `device_android_id` / `yolo_model_id` 改为允许 NULL |

> ⚠️ 如果你在项目于 **2026-08-02 之前**部署，更新代码后需执行数据库迁移脚本：
>
> ```bash
...
| 类别 | 更新内容 |
|------|---------|
| **YOLO 测试集评估** | 支持 YOLO 模型测试集评估功能，训练前检查验证集并提示确认 |
| **评估任务取消** | 支持取消测试集评估任务，后台检查状态和取消信号 |
| **取消按钮优化** | 取消按钮 hover 交互重构，使用 CSS 伪元素扩展悬停区域 |
| **数据库迁移** | 新增 `migration_001_add_test_metrics.sql`，YOLO 模型表新增 `test_metrics` / `test_status` 字段 |

> ⚠️ 如果你在项目于 **2026-07-28 之前**部署，更新代码后需执行数据库迁移脚本：
>
> ```bash
...
| 类别 | 更新内容 |
|------|---------|
| **YOLO 灵活接入** | YOLO 模型可选为空，降级为纯 OCR 识别；弹窗确认后继续关联 |
| **测试任务创建人** | 测试任务创建人从写死改为当前登录用户 |
| **测试任务详情** | 测试任务列表改用弹窗展示 Job 详情；监控页增加 YOLO/OCR 模型显示 |

### 2026-07-13 — UI 重构与双通道感知

| 类别 | 更新内容 |
|------|---------|
| **UI 统一重构** | 整体 UI 重构，登录页、列表页风格统一 |
| **Android 双通道感知** | DOM 快通道 + 视觉通道双方案，uiautomator2 dump 控件树，自动降级到 YOLO+OCR |
| **LLM 测试连接** | 支持在配置 LLM 凭证时一键测试连接可用性 |
| **指标卡片美化** | 测试集指标 SwiftUI 风格卡片；数据集提示条渐变背景 |
| **标注优化** | 标注页面提示信息可关闭，localStorage 持久化 |
