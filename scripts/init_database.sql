...
CREATE TABLE `test_plan` (
  ...
  `enable_notification`      TINYINT(1)   NOT NULL DEFAULT 0 COMMENT '是否发送通知',
  `notify_on_failure_only`   TINYINT(1)   NOT NULL DEFAULT 0 COMMENT '仅失败时通知',
  `wecom_webhooks`           JSON         NULL COMMENT '企微机器人webhook列表',
  `lark_webhooks`            JSON         NULL COMMENT '飞书机器人webhook列表',
  `dingtalk_webhooks`        JSON         NULL COMMENT '钉钉机器人webhook列表',
...
CREATE TABLE `yolo_models` (
  ...
  `metrics`      JSON          NULL     COMMENT '模型指标（验证集）',
  `test_metrics` JSON          NULL     COMMENT '测试集指标',
  `test_status`  VARCHAR(20)   DEFAULT 'untested' COMMENT '测试集评估状态(untested/pending/running/completed/failed)',
  `classes`      JSON          NOT NULL COMMENT '类别列表',
  `created_at` DATETIME      DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `is_deleted` INT           DEFAULT 0  COMMENT '是否删除(0=未删除, 1=已删除)',
  PRIMARY KEY (`id`),
